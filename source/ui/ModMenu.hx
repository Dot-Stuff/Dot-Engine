...
class ModMenu extends ui.OptionsState.Page
{
	...
	{
		var modDatas = ModHandler.getAllMods().filter(function(m)
		{
			return m != null;
		});
		var loadedModIds = ModHandler.getConfiguredMods();
	...
	public function modPreferencesChanged():Bool
	{
		var loadedModIds:Array<String> = grpMods.listCurrentMods().map(function(mod:ModMetadata) return mod.id);
		return FlxG.save.data.modConfig == loadedModIds.join('~');
	}
